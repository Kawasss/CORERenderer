    ...
    public class COREConsole
    {
        ...
        public void ParseInput(string input)
        {
            ...
            else if (input.Contains("save scene as"))
            {
                ...
                Writers.GenerateCRS(COREMain.pathRenderer, filename, $"Generated by CORE-Renderer {COREMain.VERSION}", COREMain.GetCurrentScene.models.ToArray());
            }

            else if (input == "save as stl") //debug
            {
                COREMain.GetCurrentScene.currentObj = 0;
                if (COREMain.GetCurrentScene.models.Count > 0 && COREMain.GetCurrentObjFromScene != -1)
                {
            ...
            else if (input.Length > 13 && input[..4] == "move")
            {
                try
                {
                    if (input.Contains("model["))
                    {
                        ...
                        Model model = COREMain.GetCurrentScene.models[modelIndex];
                    ...
                    else if (input.Contains("models"))
                    {
                        ...
                        if (input[4..6] == "to") //sorts for moveto, this value is applied absolutely
                        {
                            Vector3 distance = location - COREMain.GetCurrentScene.models[0].translation;
                            foreach (Model model in COREMain.GetCurrentScene.models)
                                model.translation += distance;
                            WriteLine($"Moved model to {location.x}, {location.y}, {location.z}");
                        }
                        else //sorts for move, this value is applied relatively
