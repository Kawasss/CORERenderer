...
namespace CORERenderer.OpenGL
{
    ...
    public partial class Rendering
    {
        public static Cubemap GenerateEmptyCubemap(int width, int height)
        {
            ...
            for (int i = 0; i < 6; i++)
            {
                glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, GL_DEPTH_COMPONENT, width, height, 0, GL_DEPTH_COMPONENT, GL_FLOAT, null);//reused from HDRTexture.cs, dont know if itll work well
